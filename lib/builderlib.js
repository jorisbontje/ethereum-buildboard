...
BuilderLib.isInternal = function(name) {
    return (name === 'buildbot' || _.startsWith(name, 'buildslave-') || _.startsWith(name, 'lib') || _.startsWith(name, 'qtwebengine'));
};

BuilderLib.filters = ['project', 'arch', 'branch'];

BuilderLib.archs = ['ARM', 'Linux', 'OSX', 'Windows'];

BuilderLib.getArch = function(name) {
    if (_.startsWith(name, "ARM ")) {
        return "ARM";
    } else if (_.startsWith(name, "Linux ")) {
        return "Linux";
...
BuilderLib.branches = ['master', 'develop', 'pull-requests', 'integration'];
...
BuilderLib.getBranch = function(name) {
    ...
    } else if (_.endsWith(name, " integration")) {
        return "integration";
    } else {
        return "unknown";
    }
...
BuilderLib.projects = ['go-ethereum', 'cpp-ethereum', 'pyethereum', 'serpent'];
...
BuilderLib.updateBuilder = function(name, builder) {
    ...
    var arch = BuilderLib.getArch(name);
